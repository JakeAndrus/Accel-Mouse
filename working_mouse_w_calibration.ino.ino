...
float smoothArrayVelocityX[filterSamples];
float smoothArrayVelocityY[filterSamples];
float smoothArrayLinearAccelX[filterSamples];
float smoothArrayLinearAccelY[filterSamples];
...
bool printVelocityY              = 1;
bool printAngleX                 = 0;
bool printAngleY                 = 0;
bool printGravity                = 0;
bool printCalibrateProcess       = 1;
bool printCalibrateTotal         = 1;
bool printMouseMoveX             = 0;
bool printMouseMoveY             = 0;
bool toggle                      = 0;

float Cap_xLinearAccel, Cap_yLinearAccel, Cap_xVelocity, Cap_yVelocity;
...
void loop() 
{
  ...
  rawAccelX       = event.acceleration.x;
  rawAccelY       = event.acceleration.y;
  rawAccelZ       = event.acceleration.z;
  
  gyro.getEvent(&event);
  rawGyroX        = event.gyro.x;
  rawGyroY        = event.gyro.y;
  
    //smooth all sensor data
  smoothAccelX    = (digitalSmooth(rawAccelX, smoothArrayAccelX)) - xAccelOffset;
  smoothAccelY    = (digitalSmooth(rawAccelY, smoothArrayAccelY)) - yAccelOffset;
  smoothAccelZ    = (digitalSmooth(rawAccelZ, smoothArrayAccelZ));
  smoothGyroX     = (digitalSmooth(rawGyroX,  smoothArrayGyroX))  - xGyroOffset;
  smoothGyroY     = (digitalSmooth(rawGyroY,  smoothArrayGyroY))  - yGyroOffset;
  
    //find other movement values
  xAngle          = Angle(xAxis,  smoothGyroX, smoothAccelY, smoothAccelX, smoothAccelZ, xAngle);
  yAngle          = Angle(yAxis, -smoothGyroY, smoothAccelX, smoothAccelY, smoothAccelZ, yAngle);      //Y axis gyro data must be switched to match accelerometer angle data
  
  xLinearAccel    = linearAccel(yAngle, smoothAccelX);
  yLinearAccel    = linearAccel(xAngle, smoothAccelY);

  xVelocity       = Velocity(xAxis, xLinearAccel, xVelocity);
  yVelocity       = Velocity(yAxis, yLinearAccel, yVelocity);
...
void calibrate()
{
  ...
  xAccelOffset = xAccelTotal/30;
  yAccelOffset = yAccelTotal/30;
  zAccelOffset = zAccelTotal/30;
  xGyroOffset  = xGyroTotal/30;
  yGyroOffset  = yGyroTotal/30;
